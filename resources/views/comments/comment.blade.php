<article id="comment{!! $comment->id !!}" class="comment">
    ...
    @if (user())
        @if (user()->hasAccess('comments', PERM_UPDATE) or $comment->creator->id == user()->id)
            {!! HTML::link(URL::route('comments.edit', [$comment->id]), trans('app.edit'), ['class' => 'edit btn', 'data-id' => $comment->id]) !!}
        @endif
        @if (user())
            {!! HTML::link(URL::to('#', [$comment->id]), trans('app.quote'), ['class' => 'quote btn', 'data-id' => $comment->id]) !!}
        @endif
        @if (user()->hasAccess('comments', PERM_DELETE) or $comment->creator->id == user()->id)
            {!! HTML::link(URL::route('comments.delete', [$comment->id]).'?method=DELETE', trans('app.delete'), ['class' => 'delete btn', 'data-id' => $comment->id]) !!}
        @endif
    @endif
</article>